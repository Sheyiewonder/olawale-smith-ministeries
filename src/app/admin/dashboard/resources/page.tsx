...
function getResourceThumbnail(
   ...
   *
   * This is always the strongest source because it represents
   * the thumbnail intentionally selected for the resource.
   */
  ...
  /*
   * 2. Prefer a persisted PDF thumbnail.
   *
   * PDF thumbnails are now generated by the backend and stored
   * on MediaAsset.thumbnailUrl.
   */
  const pdfThumbnail = resource.media?.find(
    (media) =>
      media.type === "PDF" &&
  ...
  /*
   * 3. Use any other persisted media thumbnail.
   *
   * This supports future media types that may have their own
   * thumbnailUrl stored in the database.
   */
  const mediaThumbnail = resource.media?.find(
    (media) => Boolean(media.thumbnailUrl),
  )?.thumbnailUrl;
   ...
   * 4. YouTube fallback.
   *
   * YouTube videos don't have a Cloudinary thumbnail, so derive
   * the standard YouTube thumbnail when necessary.
   */
  ...
  /*
   * 5. No thumbnail available.
   *
   * The caller will render the resource type icon.
   */
  return undefined;
}
          ...
          return (
            ...
            >
              <div className="flex items-start gap-4">
                {/* Thumbnail */}
                <div className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden bg-bronze/10 text-bronze">
                  {thumbnailUrl ? (
                    <img
                      src={thumbnailUrl}
                      alt=""
